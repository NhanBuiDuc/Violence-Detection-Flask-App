...
import numpy as np
from Feature_Extractor.utils.utils import (load_numpy, load_pickle, make_path, write_numpy,
                         write_pickle)
...
class BaseExtractor(object):
    ...
    def _extract(self, rgb, rgb_stack):
        """A wrapper around self.extract. It handles exceptions, checks if files already exist and saves
        the extracted files if a user desires.
        ...
        # the try and except structure is used to continue extraction even if after an error (a few bad vids)
        try:
            # extracts features for a RGB frame
            feats_dict = self.extract(rgb, rgb_stack)
            return feats_dict
                
        except KeyboardInterrupt:
            raise KeyboardInterrupt
        except:
            print(f'An error occurred during extraction from: {rgb}:')
            traceback.print_exc()  # prints the error
            print('Continuing...')
